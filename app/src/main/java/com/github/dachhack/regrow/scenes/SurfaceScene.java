...
import com.github.dachhack.regrow.sprites.RatSprite;
...
public class SurfaceScene extends PixelScene {
	...
	public void create() {
		...
		for (int i = 0; i < NCLOUDS; i++) {
			Cloud cloud = new Cloud((NCLOUDS - 1 - i) * (range / NCLOUDS)
					+ Random.Float(range / NCLOUDS), dayTime);
			window.add(cloud);
		}
		...
		for (int i = 0; i < nPatches * 4; i++) {
			GrassPatch patch = new GrassPatch((i - 0.75f) * GrassPatch.WIDTH
					/ 4, SKY_HEIGHT + 1, dayTime);
		...
		gameOver.setPos(frame.x + FRAME_MARGIN_X * 2, frame.y + frame.height
				+ 4);
		add(gameOver);
