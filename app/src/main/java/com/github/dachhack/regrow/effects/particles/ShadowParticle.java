 ...
 */
package com.github.dachhack.regrow.effects.particles;
...
public class ShadowParticle extends PixelParticle.Shrinking {
	...
	public static final Emitter.Factory MISSILE = new Factory() {
		...
		public void emit(Emitter emitter, int index, float x, float y) {
			((ShadowParticle) emitter.recycle(ShadowParticle.class))
					.reset(x, y);
		}
	};
	...
	public static final Emitter.Factory UP = new Factory() {
		@Override
		public void emit(Emitter emitter, int index, float x, float y) {
			((ShadowParticle) emitter.recycle(ShadowParticle.class)).resetUp(x,
					y);
		}
	};
