...
import com.github.dachhack.regrow.Chrome;
import com.github.dachhack.regrow.effects.ShadowBox;
...
public class Window extends Group implements Signal.Listener<Key> {
	...
	public Window(int width, int height, NinePatch chrome) {
		...
		blocker = new TouchArea(0, 0, PixelScene.uiCamera.width,
		...
		shadow.am = 0.5f;
		shadow.camera = PixelScene.uiCamera.visible ? PixelScene.uiCamera
				: Camera.main;
		add(shadow);

		chrome.x = -chrome.marginLeft();
		chrome.y = -chrome.marginTop();
		chrome.size(width - chrome.x + chrome.marginRight(), height - chrome.y
				+ chrome.marginBottom());
		add(chrome);

		camera = new Camera(0, 0, (int) chrome.width, (int) chrome.height,
